...
### Description
Add support for BBCode formatting in album and image titles. Allow common BBCode tags like `[b]` for bold, `[i]` for italic, `[u]` for underline, etc. to be parsed and rendered as HTML formatting. This enables users to add basic formatting to titles stored in the Gallery 2 database without requiring HTML knowledge.

### Requirements

#### Research Tasks
- Research BBCode parsing libraries for JavaScript/TypeScript (e.g., bbcode-parser, bbcodejs)
- Review existing BBCode implementations and best practices
- Research security considerations (XSS prevention, sanitization)
- Review where titles are displayed (AlbumCard, AlbumDetail, Lightbox)
- Consider performance implications of parsing BBCode on render
- Research accessibility implications of formatted text

#### Implementation Tasks
- Choose or create BBCode parsing utility/library
- Create utility function to parse BBCode to React elements or HTML
- Implement BBCode tag support: `[b]`, `[i]`, `[u]`, `[s]` (strikethrough), `[color]`, `[size]`
- Add sanitization to prevent XSS attacks
- Update AlbumCard component to render parsed BBCode in titles
- Update AlbumDetail component to render parsed BBCode in album titles
- Update Lightbox component to render parsed BBCode in image titles
- Handle edge cases: unclosed tags, nested tags, invalid tags
- Ensure proper escaping of literal square brackets in titles
- Add support for closing tags: `[/b]`, `[/i]`, `[/u]`, etc.
- Consider caching parsed results for performance

### Deliverable
BBCode parsing utility and updated title rendering components that support BBCode formatting

### Testing Requirements
- Verify bold text `[b]text[/b]` renders correctly
- Verify italic text `[i]text[/i]` renders correctly
- Verify underline text `[u]text[/u]` renders correctly
- Test nested tags: `[b][i]text[/i][/b]`
- Test unclosed tags (should handle gracefully)
- Test literal brackets: `[[b]]` should not be parsed
- Verify XSS prevention (malicious tags are sanitized)
- Test in AlbumCard, AlbumDetail, and Lightbox components
- Verify accessibility (screen readers handle formatted text correctly)
- Test performance with long titles containing many BBCode tags

### Technical Notes
- Titles are displayed in: AlbumCard.tsx (album titles), AlbumDetail.tsx (album titles), Lightbox.tsx (image titles)
- Must sanitize output to prevent XSS attacks
- Consider using `dangerouslySetInnerHTML` or React elements for rendering
- BBCode parsing should be efficient and not block rendering
- May want to support a subset of BBCode tags initially (bold, italic, underline)
- Consider adding configuration to enable/disable BBCode support
- Ensure proper escaping of HTML entities in non-BBCode text
- BBCode tags should be case-insensitive: `[B]` same as `[b]`
