...
## Fix Album Card Height Uniformity
...
### Description
Fix album card height inconsistency by ensuring all album cards in a grid have the same height. Cards currently have varying heights based on title length (single-line titles create shorter cards than multi-line titles). Expand `.album-card-content` to fill the remaining space in the parent card container so all cards have uniform height regardless of title length.

### Requirements

#### Research Tasks
- Review AlbumCard component structure and CSS layout
- Analyze current flexbox layout implementation
- Review `.album-card-content` CSS styling and positioning
- Check how grid layout affects card heights
- Review AlbumGrid component to understand grid container behavior
- Analyze responsive behavior across different screen sizes
- Check if list view mode needs similar fixes

#### Implementation Tasks
- Update `.album-card-content` CSS to expand to fill available space
- Use flexbox `flex: 1` property on `.album-card-content` to make it grow
- Ensure parent `.album-card` maintains proper flex layout
- Verify thumbnail area maintains fixed aspect ratio
- Test that content area expands correctly while maintaining proper spacing
- Ensure title text alignment and spacing remain correct
- Verify padding and gap spacing are maintained
- Test responsive behavior on mobile and desktop
- Verify list view mode still works correctly (if applicable)

### Deliverable
Album cards with uniform height regardless of title length, with content area expanding to fill card space

### Testing Requirements
- Verify all album cards in a grid row have the same height
- Test with albums that have single-line titles (e.g., "25c3")
- Test with albums that have multi-line titles (e.g., "Chaos Community Camp 2007")
- Verify content area expands to fill available space
- Test that thumbnail area maintains correct aspect ratio
- Verify title text is properly positioned within expanded content area
- Test responsive behavior across different screen sizes
- Verify no layout shifts or visual glitches
- Test with various numbers of cards per row

### Technical Notes
- AlbumCard component structure: `<article class="album-card">` contains `.album-card-thumbnail` and `.album-card-content`
- Current CSS: `.album-card` uses `display: flex` with `flex-direction: column`
- `.album-card-thumbnail` has fixed `aspect-ratio: 16 / 9`
- `.album-card-content` currently only has padding and flex-direction, doesn't expand
- Solution: Add `flex: 1` to `.album-card-content` to make it expand and fill remaining space
- May need to adjust title alignment (e.g., `justify-content: space-between` or similar) if needed
- List view mode (`.album-card-list`) uses horizontal layout and may need different handling
- Ensure content area doesn't overflow or cause layout issues
